...
|---|---|---|---|
| `GET` | `/skill-manager/list` | — | Returns `{ skills: SkillSummary[] }` — one entry per skill name (grouped across roots), each with a merged `locations[]`. |
...
| `POST` | `/skill-manager/write` | `SkillWriteRequest` (`targets[]`, optional `previousTargets[]`) | Creates or updates a skill in every target; any `previousTargets` no longer targeted are removed (move). |
| `POST` | `/skill-manager/remove` | `{ name, targets[] }` | Deletes a skill from each target; returns `{ removed }`. |
